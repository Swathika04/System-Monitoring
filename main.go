...
type SystemInfoWrapper struct {
	System1Info SystemInfo `json:"Swathi"`
}
...
func getblueusage() (string, error) {
	// Run the bluetoothctl show command
	out, err := exec.Command("bluetoothctl", "show").Output()
	if err != nil {
		return "", fmt.Errorf("failed to get bluetooth information: %v", err)
	}

	// Convert output to string and filter for "Powered" line
	output := string(out)
	lines := strings.Split(output, "\n")
	for _, line := range lines {
		if strings.Contains(line, "Powered") {
			// Trim any leading/trailing whitespace and get only the value after "Powered:"
			status := strings.TrimSpace(strings.Split(line, ":")[1])
			return status, nil
		}
	}

	return "", fmt.Errorf("no 'Powered' information found")
}
...
func main() {
	...
	wsURL := "ws://swathi:8080/SystemMonitoring/serverws" // Replace with your actual WebSocket server URL
